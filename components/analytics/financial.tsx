...
export const FinancialReport =  () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            const data = await getReports("token",'financial');
        ...
        <Card className="w-[800px] shadow-md">
            ...
            <CardContent className="space-y-4">
                <Table>
                    ...
                    <TableBody>
                        {reports && Object.keys(reports).map((property) => (
                            <TableRow key={property}> 
                                <TableCell className="truncate text-xs max-w-[180px] font-mono p-1 bg-slate-100 rounded-md">{property}</TableCell>
                                <TableCell className='truncate text-xs max-w-[180px] font-mono p-1 bg-slate-100 rounded-md'>{(reports as any)[property].propertyDetails.slug}</TableCell>
                                <TableCell className='truncate text-xs max-w-[180px] font-mono p-1 bg-slate-100 rounded-md'>{(reports as any)[property].numberOfBookings}</TableCell>
                                <TableCell className="text-right truncate text-xs max-w-[180px] font-mono p-1 bg-slate-100 rounded-md">{(reports as any)[property].earnings}</TableCell>
                            </TableRow> 
                        ))}
                    </TableBody>
                </Table>       
